...
[![CI](https://github.com/carefreelove/terminal-completion/actions/workflows/ci.yml/badge.svg)](https://github.com/carefreelove/terminal-completion/actions/workflows/ci.yml)
...
```sh
git clone https://github.com/carefreelove/terminal-completion.git
...
手动安装或插件管理器也可以直接加载 `terminal-completion.plugin.zsh`。例如 Oh My Zsh：

```sh
git clone https://github.com/carefreelove/terminal-completion.git \
  "${ZSH_CUSTOM:-$HOME/.oh-my-zsh/custom}/plugins/terminal-completion"
```

将 `terminal-completion` 加入已有 `plugins=(...)` 列表即可。手动安装和插件管理器安装不需要再运行安装脚本。
...
| `terminal-completion off` | 暂停历史建议，Tab 仍可用 |
| `terminal-completion on` | 启用历史建议 |
| `terminal-completion toggle` | 切换开关 |
| `terminal-completion status` | 查看版本、状态、缓存条数 |
| `terminal-completion refresh` | 立即刷新历史缓存 |
| `terminal-completion unload` | 移除插件 hooks 并恢复仍由插件管理的方向键 widgets |
...
无效的数字配置回退到默认值。颜色、开关和长度配置对后续重绘生效；修改历史条数后运行 `terminal-completion refresh`。`TC_INIT_COMPLETION` 仅在加载时读取。

可在加载插件**之后**增加快捷键：

```zsh
bindkey -M emacs '^ ' terminal-completion-accept  # Ctrl+Space 接受
bindkey -M viins '^ ' terminal-completion-accept
bindkey -M emacs '^X^T' terminal-completion-toggle # Ctrl+X，Ctrl+T 切换
```

插件通过包装 `forward-char` 和 `vi-forward-char` 接受建议，不重写你的 keymap。若你的右方向键绑定到其他自定义 widget，请自行将所需快捷键绑定到 `terminal-completion-accept`。
...
卸载脚本仅移除它管理的 `source` 区块，并备份配置；保留项目目录和历史记录。手动配置或插件管理器安装，请从相应配置中移除加载项。自行添加的快捷键也由你移除。`unload` 保留已经初始化的原生补全和 helper 函数；重新加载后如需建议，请执行 `terminal-completion on`。
...
- 没有建议时，先检查 `terminal-completion status`、当前历史以及最短前缀设置。
...
测试在临时目录中运行，不修改个人 `.zshrc`。覆盖字面前缀、Unicode、历史筛选、配置校验、重复加载、安装卸载与权限，以及真实 PTY 下的提示颜色、接受与撤销、行中编辑、Vi 模式、Tab 路径/参数补全和其他插件显示内容的保留。
