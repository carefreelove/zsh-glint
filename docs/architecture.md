...
1. 在交互式 Zsh 5.9+ 中加载插件，入口在局部选项作用域内关闭 alias 展开，再加载 `lib/glint.zsh`；初始化默认配置，重复加载直接返回。
...
5. 转义用户前缀的模式字符，用 Zsh 原生数组搜索选取最近的较长候选；若首个匹配项超过长度上限，继续寻找符合上限的匹配项。输入和有效长度配置未变化时复用最近查询。将后缀写入 `POSTDISPLAY`，用带有 `memo=zsh-glint` 的 `region_highlight` 条目着色。
...
历史 key 排序发生在 prompt 边界，成本随 Shell 已加载的历史总量增长。缓存最多保存 `TC_HISTORY_LIMIT` 范围内的有效候选；首次查询通过 Zsh 原生数组搜索执行，最坏仍是线性扫描。相同查询可复用最近结果，刷新历史会使该结果失效。默认上限为 1000 条、512 字符，键入时不启动外部进程。实测数据与未优化部分见 [性能报告](performance.md)。
...
后加载的插件可能保存本插件的 widget，再用自己的 wrapper 接管同名 widget。卸载后直接重新包装这一调用链可能形成递归，因此检测到后续替换时，需要在新 Zsh 会话中重新加载。`zsh-glint doctor` 可检查配置、hook/widget 归属、原生补全及目录权限，报告退出码和历史条数，不输出历史命令。完整验证边界见 [兼容性说明](compatibility.md)。
