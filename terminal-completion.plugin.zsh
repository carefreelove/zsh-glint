# terminal-completion — native Zsh history suggestions and completion.
# SPDX-License-Identifier: MIT
[[ -o interactive ]] || return 0
autoload -Uz is-at-least
if ! is-at-least 5.9; then
  print -u2 -- 'terminal-completion requires Zsh 5.9 or newer.'
  return 1
fi
(( ${+_TC_LOADED} )) && return 0
...
() {
  ...
  _tc_clear() {
    ...
    region_highlight=( "${(@)region_highlight:#*memo=terminal-completion}" )
    _tc_suffix=''
    _tc_source_buffer=''
    return 0
  }
  ...
  _tc_redraw() {
    ...
    _tc_suffix=$REPLY
    POSTDISPLAY=$REPLY
    region_highlight+=( "${#BUFFER} $(( ${#BUFFER} + ${#REPLY} )) $TC_STYLE memo=terminal-completion" )
    return 0
  }
  ...
  terminal-completion() {
    ...
    case ${1:-status} in
      ...
      status) print -r -- "terminal-completion 0.1.0 | enabled=$TC_ENABLED | history=${#_tc_history} | zsh=$ZSH_VERSION" ;;
      ...
      unload)
        ...
        unset _TC_LOADED
        _tc_history=()
        print -- 'terminal-completion unloaded; native completion remains available.'
        ;;
      help|--help|-h)
        print -- 'Usage: terminal-completion [on|off|toggle|status|refresh|unload]'
        ;;
